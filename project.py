...
def fbconnect():
    return oauth.fbconnect()


@app.route('/fbdisconnect')
def fbdisconnect():
    return oauth.fbdisconnect()

@app.route('/gconnect', methods=['POST'])
def gconnect():
    return oauth.gconnect()

# DISCONNECT - Revoke a current user's token and reset their login_session
@app.route('/gdisconnect')
def gdisconnect():
    return oauth.gdisconnect()
...
def showRestaurants():
    template = 'public_restaurants.html'
    print login_session
...
def showMenu(restaurant_id):
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    template = 'public_menu.html'
    if 'user_id' in login_session and restaurant.user_id  == login_session['user_id']:
        template = 'menu.html'
    items = session.query(MenuItem).filter_by(
        restaurant_id=restaurant_id).all()
    return render_template(template, items=items, restaurant=restaurant, creator=restaurant.user)
